...
import { useLoginValidation } from "../utils/auth/validations";
import { loginData } from "../types/auth.type";

const Login = () => {
  const { register, handleSubmit, errors, isSubmitting, isDirty, isValid } = useLoginValidation()
  ...
  return (
    <div className="relative overflow-hidden">
      ...
      <div className="relative flex justify-center w-screen h-screen">
        <form 
          className="absolute top-[52%] left-[50%] -translate-x-1/2 -translate-y-1/2 z-50 w-[70%] bg-white/[.935] p-5 pt-7 rounded-lg drop-shadow-md"
        ...
        >
          <div className="relative mb-4" data-te-input-wrapper-init>
            ...
            <label
              ...
              className="absolute left-0 pl-0.5 -top-3.5 text-gray-600 text-sm peer-placeholder-shown:text-base peer-placeholder-shown:text-gray-440 peer-placeholder-shown:top-2 transition-all peer-focus:-top-3.5 peer-focus:text-gray-600 peer-focus:text-sm"
            >
              Email address
            </label>
          </div>
          <div className="relative mb-[3rem]" data-te-input-wrapper-init>
            <input
              type="password"
              {...register("password")}
              className={`peer block min-h-[auto] w-full pl-1 rounded border-b border-gray-500 bg-transparent py-[0.32rem] leading-[1.8] outline-none transition-all duration-200 ease-linear focus:placeholder:opacity-100 data-[te-input-state-active]:placeholder:opacity-100 motion-reduce:transition-none dark:text-neutral-200 dark:placeholder:text-neutral-200 [&:not([data-te-input-placeholder-active])]:placeholder:opacity-0 ${!(errors.password?.message) && "mb-[1.5rem]"}`}
              id="exampleFormControlInput22"
              placeholder="Password"
            />
            {errors.password?.message &&
              <p className="mb-[1.5rem] text-red-500">{errors.password?.message}</p> 
            }
            <label
